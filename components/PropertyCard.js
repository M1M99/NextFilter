    ...
    return (
        <div key={property.id} className="p-4 rounded-lg shadow shadow-blue-600">
            <Link href={`/${property.id}`}>
                <img src={property.imageUrl} style={{ width: `${width}px`, height: `${height}px` }} alt={property.title} className="rounded mb-3" />
                ...
                <p className="text-sm text-gray-500">Date : {property.createdAt}</p>
            </Link>
        </div>
    )
}